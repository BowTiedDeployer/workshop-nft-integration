...
/** @const */
var IS_IOS =
  (!!window.navigator.userAgent.match(/iP(hone|ad|od)/i) && !!window.navigator.userAgent.match(/Safari/i)) ||
  _isIpad() ||
...
Runner.prototype = {
  ...
  loadImages: function () {
    ...
    var nft_img_dom = document.getElementById('playable-nft');
    nft_img_dom.src = `https://stacksgamefi.mypinata.cloud/ipfs/Qmd6mjXARXZcCvdDPeisAHMEQgKW4aw85rktZ5yFAKYNbP/${localStorage.getItem(
      'selectedNFT'
    )}.png`;

    // nft_img_dom.src = './images/nft_basic.png';
    Runner.imageNFT = nft_img_dom;
    this.spriteDef = Runner.spriteDefinition;

    if (Runner.imageSprite.complete && Runner.imageNFT.complete) {
      this.init();
    } else {
      // If the images are not yet loaded, add a listener.
      // Listener on in-game because it takes more to get the in-game nft than the local assets
      Runner.imageNFT.addEventListener(Runner.events.LOAD, this.init.bind(this));
      // Runner.imageNFT.addEventListener(Runner.events.LOAD, this.init.bind(this)); // this doubles the canvas
    }
  },
  ...
  onKeyUp: function (e) {
    ...
    if (h1) h1.innerHTML = 'Press Esc to exit';
  ...
  isLeftClickOnCanvas: function (e) {
    return e.button != null && e.button < 2 && e.type == Runner.events.POINTERUP && e.target == this.canvas;
  },
...
function DistanceMeter(canvas, spritePos, canvasWidth) {
  this.canvas = canvas;
  this.canvasCtx = canvas.getContext('2d');
  this.imageSprr = Runner.imageSprite;
  // this.imageCustom = Runner.imageNFT;
  this.spritePos = spritePos;
  this.x = 0;
  this.y = 5;
...
DistanceMeter.prototype = {
  ...
  getHighScoreBounds: function () {
    return {
      ...
      y: this.y,
      width:
        DistanceMeter.dimensions.WIDTH * (this.highScore.length + 1) + DistanceMeter.config.HIGH_SCORE_HIT_AREA_PADDING,
...
NightMode.prototype = {
  ...
  draw: function () {
    ...
    var starSourceX = Runner.spriteDefinition.STAR.x;
  ...
  placeStars: function () {
    ...
    for (var i = 0; i < NightMode.config.NUM_STARS; i++) {
      ...
      this.stars[i].sourceY = Runner.spriteDefinition.STAR.y + NightMode.config.STAR_SIZE * 2 * i;
    }
  },
...
function onDocumentLoad() {
  document.body.setAttribute('class', 'offline');
